...
std::vector<std::vector<std::int32_t>> Labeling_omp(
  ...
  #pragma omp parallel
  {
    ...
    if (thread_count >= h)
      ...
      first_row = (thread_id == 0 ? 0 : h / thread_count + h % thread_count);
    #pragma omp for
  ...
  if (thread_count <= h) {
    ...
    str_ind = h % thread_count + shift;
  }

  std::cout << "shift " << shift << std::endl;
  std::cout << "str ind " << str_ind << std::endl;
  ...
  void Merge_omp(std::vector<std::vector<std::int32_t>> * ptr,
                 std::int32_t str_row, std::int32_t pr_num,
                 std::int32_t req_num, std::int32_t end_x, std::int32_t end_y) {
