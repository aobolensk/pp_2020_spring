...
//   mtrxmult::Matrix mult_res_cannon =
//       mtrxmult::multiply_cannon(left, right, SEQUENTIAL);
...
//   ASSERT_TRUE(mtrxmult::multiply(test_matrix, test_matrix_2, PARALLEL)
//                   .equals(mult_result));
// }
...
//   mtrxmult::Matrix mult_result(
//       {9.0, 12.0, 17.0, 19.0, 26.0, 37.0, 29.0, 40.0, 57.0, 39.0,
//       54.0, 77.0}, 4, 3);
...
// TEST(Matrix_Mult_Seq, Mult_Cannon_Rand_80x80) { test(800); }

TEST(Sample, test1) {
  mtrxmult::Matrix test_matrix({1, 2, 3, 4}, 2, 2);
  mtrxmult::Matrix test_matrix_2({1, 2, 3, 4}, 2, 2);

  mtrxmult::Matrix res1, res2;
  double time;

  omp_set_num_threads(4);
  time = omp_get_wtime();
  res2 = mtrxmult::multiply(test_matrix, test_matrix_2, PARALLEL);
  std::cout << "Elapsed time seq: " << omp_get_wtime() - time << "s"
            << std::endl;

  omp_set_num_threads(4);
  time = omp_get_wtime();
  res1 = mtrxmult::multiply_cannon(&test_matrix, &test_matrix_2);
  std::cout << "Elapsed time parll: " << omp_get_wtime() - time << "s"
            << std::endl;

  std::cout << test_matrix.to_string() << std::endl;
  std::cout << test_matrix_2.to_string() << std::endl;

  std::cout << res1.to_string() << std::endl;
  std::cout << res2.to_string() << std::endl;

  ASSERT_TRUE(res1.equals(res2));
}
