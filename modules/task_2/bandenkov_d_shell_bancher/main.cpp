...
TEST(test_Shell_Betch_sort_omp, test_on_20_elements_2_thread) {
  std::vector<int> res = getRandomVector(20);
  std::vector<int> betch = shellBatcher_omp(res, 2, 20);
  std::sort(res.begin(), res.end());
  ASSERT_EQ(betch, res);
}
...
TEST(test_Shell_Betch_sort_omp, test_on_1000_elements_2_thread) {
  std::vector<int> res = getRandomVector(1000);
  std::vector<int> betch = shellBatcher_omp(res, 2, 1000);
  std::sort(res.begin(), res.end());
  ASSERT_EQ(betch, res);
}

TEST(test_Shell_Betch_sort_omp, test_on_1000_elements_4_thread) {
  std::vector<int> res = getRandomVector(1000);
  std::vector<int> betch = shellBatcher_omp(res, 4, 1000);
  std::sort(res.begin(), res.end());
  ASSERT_EQ(betch, res);
}
...
TEST(test_Shell_Betch_sort_omp, test_on_5000_elements_4_thread) {
  ...
  std::vector<int> betch = shellBatcher_omp(res, 4, 5000);
  std::sort(res.begin(), res.end());
  ASSERT_EQ(betch, res);
}
...
TEST(test_Shell_Betch_sort_omp, test_on_5000_elements_6_thread) {
  std::vector<int> res = getRandomVector(5000);
  std::vector<int> betch = shellBatcher_omp(res, 6, 5000);
  std::sort(res.begin(), res.end());
  ASSERT_EQ(betch, res);
}
