...
void radixSortOddEvenMergeDouble(double* array, int index, int size) {
    ...
    //         std::cout<<"???\n"+std::to_string(index) + " " + std::to_string(size) +" "+std::to_string(i) + " "+ std::to_string(array[i]) + " " +std::to_string(array[i+1])+"\n";
    //     }
    // }
}
...
void gen_tasks(double* array, int left, int right, const std::vector<task*>& tasks, const std::vector<int>& portion, task* queue[], bool is_begin) {
    if (left == right)
    {
...
std::vector<double> radixSortOddEvenMergeDoubleParallel(const std::vector<double>& v) {
    ...
    for (int i = 0; i < num_threads; i++) {
        portion[i] = static_cast<int>(v.size())/num_threads + (i < (static_cast<int>(v.size())%num_threads)?1:0) + ((i==0)?0:portion[i-1]);
