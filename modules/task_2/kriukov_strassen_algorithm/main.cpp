...
TEST(OMPStrassen, Test_Strassen_Mult) {
  ...
  EXPECT_CONTAINER_DOUBLE_EQ(std::vector<double>, strassenMultiplication_omp(a, b, 2), c);
}
...
TEST(OMPStrassen, Test_Strassen_Mult_size_8) {
  ...
  std::vector<double> res_strassen = strassenMultiplication_omp(a, b, sz);
...
TEST(OMPStrassen, Time_Test_Strassen_Mult_size_256) {
  ...
  std::vector<double> res_parallel = strassenMultiplication_omp(a, b, sz);
  clock_t end_s = clock();
