...
TEST(Sequential_Contrast_Enhancement, Test_Result_Matrix_Size_Negative) {
    ...
    ASSERT_ANY_THROW(getResultMatrixTbb(matrix, matrixWidth, matrixHeight));
}
...
TEST(Sequential_Contrast_Enhancement, Test_Result_Matrix_Size_Correctly) {
    ...
    ASSERT_NO_THROW(getResultMatrixTbb(matrix, matrixWidth, matrixHeight));
}
...
TEST(Sequential_Contrast_Enhancement, Test_Check_Result_Matrix_Correct_100x100) {
    ...
    std::vector<int> resultTbb(width * height);
    ...
    // unsigned int startTimeTbb = clock();
    resultTbb = getResultMatrixTbb(matrix, width, height);
    // unsigned int endTimeTbb = clock();
    // std::cout << "SEQ: " << endTimeSeq - startTimeSeq << std::endl;
    // std::cout << "TBB: " << endTimeTbb - startTimeTbb << std::endl;
...
TEST(Sequential_Contrast_Enhancement, Test_Check_Result_Matrix_Correct_4x4) {
    ...
    ASSERT_EQ(result, getResultMatrixTbb(initial, width, height));
}
