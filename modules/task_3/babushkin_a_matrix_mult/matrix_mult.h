...
#ifndef MODULES_TASK_3_BABUSHKIN_A_MATRIX_MULT_MATRIX_MULT_H_
#define MODULES_TASK_3_BABUSHKIN_A_MATRIX_MULT_MATRIX_MULT_H_
...
class Matrix {
 ...
 public:
  ...
  double at(int row, int col) const { return m_data.at(row * m_cols + col); }
  int get_rows() const { return m_rows; }
  int get_cols() const { return m_cols; }
  size_t get_size() const { return m_data.size(); }

  std::vector<double> get_data() const { return std::vector<double>(m_data); }
...
#endif  // MODULES_TASK_3_BABUSHKIN_A_MATRIX_MULT_MATRIX_MULT_H_
