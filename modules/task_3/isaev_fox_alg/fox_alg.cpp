...
Matrix foxAlgParallel(const Matrix& mat1, const Matrix& mat2) {
    ...
    auto body = [&](const tbb::blocked_range2d<size_t>& it) {
        auto thread_i = it.rows().begin() / block_size;
        auto thread_j = it.cols().begin() / block_size;
    ...
    tbb::parallel_for(tbb::blocked_range2d<size_t>(0, n, block_size, 0, n, block_size), body);
    tbb::parallel_for(tbb::blocked_range<size_t>(0, n), resize_if_changed);
    if (old_n != n)
        res.resize(old_n);
