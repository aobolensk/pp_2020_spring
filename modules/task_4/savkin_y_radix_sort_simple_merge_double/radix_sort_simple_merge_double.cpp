...
#include <functional>
#include <algorithm>
#include "../../../modules/task_4/savkin_y_radix_sort_simple_merge_double/radix_sort_simple_merge_double.h"
...
void radixSortForDouble(double* arr, size_t count, size_t num_threads) {
     ...
     public:
        ...
        size_t execute() {
            if (parts_count < 2) {
                ...
                w.push_back(vec->size() + 1);
                RadixTaskInfo crti {new SortFunctor(reinterpret_cast<radixDouble*>(arr[0]), parts_sizes[0],
                    reinterpret_cast<radixDouble*>(out[0])), w, 0};
                vec->push_back(crti);
                RadixTaskInfo rti {new RewriteFunctor(arr[0], out[0], parts_sizes[0]), v, 1};
                vec->push_back(rti);
            ...
            if (parts_count > 2) {
                ...
                std::vector<size_t> w;
                w.push_back(vec->size() + 2);
                RadixTaskInfo crti1 {new SortFunctor(reinterpret_cast<radixDouble*>(arr[0]), parts_sizes[0],
                    reinterpret_cast<radixDouble*>(out[0])), w, 0};
                RadixTaskInfo crti2 {new SortFunctor(reinterpret_cast<radixDouble*>(arr[parts_half]),
                    parts_sizes[parts_half], reinterpret_cast<radixDouble*>(out[parts_half])), w, 0};
                vec->push_back(crti1);
                vec->push_back(crti2);
            }
            ...
            RadixTaskInfo rti {new MergeFunctor(arr1, arr2, size1, size2, output), v, 2};
            vec->push_back(rti);
    ...
    if (num_threads > 1) {
        ...
        std::thread** threads_arr = new std::thread*[num_threads - 1];
        std::mutex mute;
        ...
        std::function<void()> func = [&]()->void {
            ...
            while (1) {
                ...
                if (my_task.second == nullptr) {
                    if (active_threads > info_vec.size() - solved_tasks) {
                        --active_threads;
                        end = true;
                    }
                }
        ...
        for (size_t i = 0; i < num_threads - 1; ++i) {
            ...
            threads_arr[i] = new std::thread(func2);
        }
        ...
        SortFunctor sf(reinterpret_cast<radixDouble*>(arr), count,
