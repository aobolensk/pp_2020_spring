...
#include <../../../modules/task_4/kriukov_strassen_algorithm/strassen_algorithm.h>
#include <vector>
...
TEST(STDStrassen, Test_Strassen_Mult) {
  std::vector<double> a = { 1.0, 2.0, 3.0, 4.0 };
  std::vector<double> b = { 1.0, 2.0, 3.0, 4.0 };
  std::vector<double> c = { 7.0, 10.0, 15.0, 22.0 };

  EXPECT_CONTAINER_DOUBLE_EQ(std::vector<double>, strassenMultiplication_std(a, b, 2), c);
}

TEST(STDStrassen, Test_Strassen_Mult_size_8) {
  unsigned int sz = 8;
  ...
  std::vector<double> res_strassen = strassenMultiplication_std(a, b, sz);

  EXPECT_CONTAINER_DOUBLE_EQ(std::vector<double>, res_regular, res_strassen);
}

TEST(STDStrassen, Test_Strassen_Mult_size_64) {
  unsigned int sz = 64;
  std::vector<double> a = getRandomMatrix(sz);
  std::vector<double> b = getRandomMatrix(sz);
  std::vector<double> res_regular = regularMultiplication(a, b, sz);
...
TEST(STDStrassen, Test_Strassen_Mult_size_3) {
  unsigned int sz = 3;
  std::vector<double> a = getRandomMatrix(sz);
  std::vector<double> b = getRandomMatrix(sz);
  std::vector<double> res_regular = regularMultiplication(a, b, sz);
  ...
  TEST(STDStrassen, Test_Strassen_Mult_size_59) {
    unsigned int sz = 59;
    std::vector<double> a = getRandomMatrix(sz);
    std::vector<double> b = getRandomMatrix(sz);
    std::vector<double> res_regular = regularMultiplication(a, b, sz);
...
TEST(STDStrassen, Time_Test_Strassen_Mult_size_256) {
  unsigned int sz = 256;
  std::vector<double> a = getRandomMatrix(sz);
  std::vector<double> b = getRandomMatrix(sz);
  ...
  std::vector<double> res_parallel = strassenMultiplication_std(a, b, sz);
  clock_t end_s = clock();
...
TEST(STDStrassen, Time_Test_Strassen_Mult_size_512) {
  unsigned int sz = 512;
  std::vector<double> a = getRandomMatrix(sz);
  std::vector<double> b = getRandomMatrix(sz);
  clock_t start_r = clock();
...
TEST(STDStrassen, Time_Test_Strassen_Mult_size_529) {
  unsigned int sz = 529;
  std::vector<double> a = getRandomMatrix(sz);
  std::vector<double> b = getRandomMatrix(sz);
  clock_t start_r = clock();
...
TEST(STDStrassen, Time_Test_Strassen_Mult_size_1024) {
...
TEST(STDStrassen, Time_Test_Strassen_Mult_size_1029) {
...
TEST(STDStrassen, Time_Test_Strassen_Mult_size_2048) {
...
TEST(STDStrassen, Time_Test_Strassen_Mult_size_4096) {
...
TEST(STDStrassen, Time_Test_Strassen_Mult_size_8192) {
