...
std::vector<double> strassenMultiplication_std(const std::vector<double>& a,
  ...
  res = strassenMultiplication_NoCastToPowerOfTwo_std(a_, b_, tpow);

  return matrixReduce(res, sz);
}

std::vector<double> strassenMultiplication_NoCastToPowerOfTwo_std(const std::vector<double>& a_,
  ...
  if (a11.size() > 256) {
    std::thread thread1([=, &p1] {
      p1 = strassenMultiplication_NoCastToPowerOfTwo(sum(a11, a22, split_sz),
    ...
    std::thread thread5([=, &p5] {
      p5 = strassenMultiplication_NoCastToPowerOfTwo(sum(a11, a12, split_sz),
        b22, split_sz);
    });
    std::thread thread6([=, &p6] {
      p6 = strassenMultiplication_NoCastToPowerOfTwo(diff(a21, a11, split_sz),
        sum(b11, b12, split_sz), split_sz);
    });
    std::thread thread7([=, &p7] {
      p7 = strassenMultiplication_NoCastToPowerOfTwo(diff(a12, a22, split_sz),
