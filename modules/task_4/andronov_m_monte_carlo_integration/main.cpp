...
TEST(MonteCarloIntegration, std_one_dims_sin) {
    const size_t dims = 1;
...
TEST(MonteCarloIntegration, std_one_dims_quadratic) {
    const size_t dims = 1;
    std::function<double(const std::vector<double>&)> integrand =
...
TEST(MonteCarloIntegration, std_two_dims_hyperboloid) {
    const size_t dims = 2;
    std::function<double(const std::vector<double>&)> integrand =
...
TEST(MonteCarloIntegration, std_two_dims_inverse_hyperboloid) {
    const size_t dims = 2;
    std::function<double(const std::vector<double>&)> integrand =
...
TEST(MonteCarloIntegration, std_three_dims_integral) {
    const size_t dims = 3;
    std::function<double(const std::vector<double>&)> integrand =
