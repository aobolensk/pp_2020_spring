...
#include <algorithm>
#include <thread>
#include "../../../modules/task_4/karin_t_sparce_matrix_complex_CCS/sparce_matrix.h"
...
SparceMatrix ParMult(const SparceMatrix& A, const SparceMatrix& B, int th_number) {
  ...
  std::vector<std::vector<int>> res_row_num(th_number);
  std::thread* thr = new std::thread[th_number];
